...
def main():
    ...
    for i, data_set in enumerate(data_sets):
        ...
        print("  "
            f'jump: {_get_pct(data_set["data"][f"jmp_hits_per_round_{METRIC}"])}%', 
            f'call: {_get_pct(data_set["data"][f"call_hits_per_round_{METRIC}"])}%, '
            f'ret: {_get_pct(data_set["data"][f"ret_hits_per_round_{METRIC}"])}%, '
            f'noise: {_get_pct(max(data_set["data"][f"jmp_check"],data_set["data"][f"call_check"],data_set["data"][f"ret_check"]))}%'
        )
